 ...
 * @author Admin
 */
public class ChangeStatusController extends BaseAuthController {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ChangeStatusController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ChangeStatusController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response, Account account)
        ...
        CV cv = cvDBContext.getCVByMentor(mentorId);
        if (cv != null) {
            String[] skills = cv.getSkills().split("\n");
            for (String skill : skills) {
              skillDBContext.insertMentorSkill(mentorId, skill.substring(2));
            }
        }
